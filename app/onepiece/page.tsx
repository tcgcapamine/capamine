...
export default async function OnePiecePage() {
  ...
  const [news, prices, events] = await Promise.all([
    prisma.article.findMany({ where: { isPublished: true, category: "onepiece" }, orderBy: { publishedAt: "desc" }, take: 13 }),
    prisma.cardPrice.findMany({ where: { category: "onepiece" }, orderBy: { price: "desc" }, take: 10 }),
    prisma.releaseEvent.findMany({ where: { category: "onepiece", releaseDate: { gte: now } }, orderBy: { releaseDate: "asc" }, take: 5 }),
  ]);
  const hero = news[0];
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", paddingBottom: "80px" }}>
      ...
      {/* ── 히어로 ── */}
      {hero ? (
        <div style={{ background: `linear-gradient(135deg, ${C.opDim} 0%, #0a0a0a 55%)`, borderBottom: `1px solid ${C.bd}` }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.25rem" }}>
            <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 320px", minHeight: "320px" }}>
              ...
              <Link href={`/articles/${hero.id}`} style={{ textDecoration: "none", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "36px 36px 36px 0", borderRight: `1px solid ${C.bd}`, position: "relative" }}>
                <span className="f-display" style={{ position: "absolute", top: "16px", left: 0, fontSize: "140px", fontWeight: 900, color: C.op, opacity: 0.04, lineHeight: 1, userSelect: "none", letterSpacing: "-0.05em" }}>
                  001
                </span>
                <div style={{ position: "relative" }}>
                  ...
                  <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                    ...
                    <span className="f-display" style={{ marginLeft: "auto", fontSize: "11px", fontWeight: 700, color: C.op, letterSpacing: "0.08em" }}>기사 보기 →</span>
                  </div>
                </div>
              </Link>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                {events.length === 0 ? (
                  ...
                  events.map((ev, i) => {
                    ...
                    const dColor = days <= 7 ? C.red : days <= 30 ? C.op : C.text3;
                    return (
                      <div key={ev.id} style={{ padding: "14px 24px", borderBottom: i < events.length - 1 ? `1px solid ${C.bd}` : "none", display: "flex", gap: "12px", alignItems: "center" }}>
                        <div style={{ textAlign: "center", flexShrink: 0, width: "32px" }}>
                          <div className="f-display" style={{ fontSize: "8px", color: C.op, fontWeight: 800 }}>{d.getMonth() + 1}월</div>
        ...
        <div style={{ background: `linear-gradient(135deg, ${C.opDim} 0%, #0a0a0a 55%)`, borderBottom: `1px solid ${C.bd}`, padding: "40px 1.25rem" }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            <span className="f-display" style={{ fontSize: "10px", fontWeight: 800, color: C.op, letterSpacing: "0.18em" }}>☠️ ONE PIECE</span>
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.25rem", display: "flex", alignItems: "stretch" }}>
            ...
            {prices.slice(0, 6).map((p, i) => (
              <div key={p.id} style={{ padding: "10px 18px", borderRight: `1px solid ${C.bd}`, flexShrink: 0 }}>
                <div style={{ fontSize: "8px", fontWeight: 800, color: C.op, letterSpacing: "0.1em", marginBottom: "2px" }}>{p.rarity ?? "ONE PIECE"}</div>
                <div className="line-clamp-1" style={{ fontSize: "11px", color: C.text2, marginBottom: "3px", maxWidth: "100px" }}>{p.cardName}</div>
                <div style={{ display: "flex", gap: "5px", alignItems: "baseline" }}>
                  <span className="f-display" style={{ fontSize: "14px", fontWeight: 900, color: C.op }}>{fmtPrice(p.price)}<span style={{ fontSize: "9px", fontWeight: 400 }}>원</span></span>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 1.25rem 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "14px", marginBottom: "16px" }}>
          <span style={{ display: "inline-block", width: "4px", height: "28px", background: C.op, flexShrink: 0 }} />
          <span className="f-headline" style={{ fontSize: "clamp(18px, 2.5vw, 24px)", color: C.text }}>원피스 카드게임</span>
          <span className="f-display" style={{ fontSize: "11px", color: C.op, fontWeight: 800, letterSpacing: "0.1em" }}>{rest.length}건</span>
          <Link href="/onepiece" className="hover-dim f-display" style={{ marginLeft: "auto", fontSize: "11px", color: C.text3, fontWeight: 700, letterSpacing: "0.1em" }}>전체보기 →</Link>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "2px", background: C.bd }} className="news-grid">
            {rest.slice(0, 6).map((a, i) => (
              <Link key={a.id} href={`/articles/${a.id}`} className="card-lift"
                style={{ textDecoration: "none", background: C.s1, padding: "18px 20px", display: "flex", flexDirection: "column", gap: "8px", borderTop: `2px solid ${C.op}` }}>
        ...
        {prices.length > 0 && (
          <div style={{ marginTop: "48px" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: "14px", marginBottom: "16px" }}>
              ...
              <Link href="/onepiece/prices" className="hover-dim f-display" style={{ marginLeft: "auto", fontSize: "11px", color: C.text3, fontWeight: 700, letterSpacing: "0.1em" }}>전체보기 →</Link>
            </div>
              ...
              {prices.map((p, i) => (
                <div key={p.id} style={{ background: C.s1, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px" }}>
                  <div style={{ display: "flex", gap: "12px", alignItems: "center", minWidth: 0 }}>
                    <span className="f-display" style={{ fontSize: "18px", fontWeight: 900, color: i < 3 ? C.op : C.text3, minWidth: "24px" }}>{i + 1}</span>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div className="f-display" style={{ fontSize: "16px", fontWeight: 900, color: C.op }}>{fmtPrice(p.price)}<span style={{ fontSize: "10px", fontWeight: 400 }}>원</span></div>
                    <Diff cur={p.price} prev={p.prevPrice} />
                  </div>
                </div>
              ))}
